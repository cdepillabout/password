...
module Data.Password.Bcrypt (
  ...
  -- /bcrypt/ algorithm, please, please just use 'hashPass'.
  , hashPassWithParams
  , hashPassWithSalt
  , newSalt
  ) where
...
-- >>> instance Arbitrary (PassHash Bcrypt) where arbitrary = hashPassWithSalt 10 <$> arbitrary <*> arbitrary
...
-- instead of generating a random salt.
--
-- __N.B.__: The salt HAS to be 16 bytes or this function will throw an error!
--
-- >>> let salt = Salt "abcdefghijklmnop"
-- >>> hashPassWithSalt 12 salt (mkPass "foobar")
-- PassHash {unPassHash = "$2b$12$WUHhXETkX0fnYkrqZU3ta.pkddbc1r8CRUHGJyJ.siL0YkdRYfVuq"}
--
...
--
-- __N.B.__: It is advised to use 'hashPass' if you're not sure what
-- the cost implies.
--
-- @since 2.0.0.0
hashPassWithParams
